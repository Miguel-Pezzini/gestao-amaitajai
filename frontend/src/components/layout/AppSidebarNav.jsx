...
export function AppMobileNav({
  sidebarItems,
  sidebarGroups,
  cadastroItemsExpanded,
  onCadastroItemsExpandedChange,
}) {
  const navigate = useNavigate();
  const location = useLocation();
  const currentPath = location.pathname;
  const cadastrosGroup = sidebarGroups[0];

  useEffect(() => {
    if (cadastrosGroup && isGroupActive(currentPath, cadastrosGroup.routePrefix)) {
      onCadastroItemsExpandedChange(true);
    }
  }, [cadastrosGroup, currentPath, onCadastroItemsExpandedChange]);
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2 overflow-x-auto">
        ...
        {cadastrosGroup ? (
          <Button
            ...
            className={cn(
              "shrink-0 gap-2",
              isGroupActive(currentPath, cadastrosGroup.routePrefix)
                ? "bg-white/15 text-white"
                : "text-white/80 hover:bg-white/10 hover:text-white",
            )}
            onClick={() => onCadastroItemsExpandedChange(!cadastroItemsExpanded)}
            aria-expanded={cadastroItemsExpanded}
          >
            <FolderCog className="size-4 shrink-0" aria-hidden="true" />
            {cadastrosGroup.label}
            <ChevronDown
              className={cn(
                "size-4 shrink-0 transition-transform",
                cadastroItemsExpanded && "rotate-180",
              )}
              aria-hidden="true"
            />
          </Button>
      ...
      {cadastrosGroup && cadastroItemsExpanded ? (
        <div className="flex gap-2 overflow-x-auto border-t border-white/10 pt-2">
          {cadastrosGroup.items.map((item) => {
            const active = isActiveRoute(currentPath, item.route);
            const Icon = item.icon;
            return (
              <Button
